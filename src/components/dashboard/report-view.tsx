    ...
    <div className="report-page">
      ...
      <article className="report-paper">
        <header className="report-cover">
          ...
          <div className="report-child"><div className="report-avatar">{child.name[0]}</div><div><small>CHILD PROGRESS SUMMARY</small><h2>{child.name}</h2><p>{getAgeString(new Date(child.birth_date))} · Generated {generated}</p></div></div>
        ...
        <section className="report-section">
          ...
          {entries.length ? <div className="report-table"><div className="report-row head"><span>Date</span><span>Weight</span><span>Length</span><span>Head</span></div>{entries.slice(-8).reverse().map((entry) => <div className="report-row" key={entry.id}><span>{new Date(entry.date).toLocaleDateString()}</span><span>{entry.weight_kg ? `${entry.weight_kg} kg` : "—"}</span><span>{entry.height_cm ? `${entry.height_cm} cm` : "—"}</span><span>{entry.head_circumference_cm ? `${entry.head_circumference_cm} cm` : "—"}</span></div>)}</div> : <p className="report-empty">No measurements have been recorded yet.</p>}
        </section>
        <section className="report-section">
          <div className="report-title"><span>DEVELOPMENT STORY</span><h3>Recent milestones</h3></div>
          <div className="report-milestones">{achieved.slice(0,6).map((item) => <div key={item.id}><i><Check /></i><div><strong>{item.title}</strong><p>{item.description || item.category.replace("_"," ")}</p><small>{item.achieved_at ? new Date(item.achieved_at).toLocaleDateString() : ""}</small></div></div>)}{!achieved.length ? <p className="report-empty">No completed milestones have been saved yet.</p> : null}</div>
        </section>
